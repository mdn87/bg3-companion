...
1. **M0 — Decide the repo shape (needs Matt).** Choose between renaming this repo or creating a new `wow-companion` repo from this code. Recommendation: **new repo**, with this one archived as `bg3-companion`. BG3 history and docs would stay coherent, and the WoW repo would start clean of input code.
